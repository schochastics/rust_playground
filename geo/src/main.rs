use csv;
use geo::algorithm::{Area, Centroid};
...
fn main() {
    let mut feature_reader = {
        use std::fs::File;
        let file = File::open("src/data/philly_waterways/philly_waterways.csv")
    ...
    for row in feature_reader.records() {
        ...
        let creek_name = creek_segment
            .get(0)
            .expect("'creek_name' field must be present");
        let infrastructure_label = creek_segment.get(1).expect("'inf1' field must be present");
        let geometry_str = creek_segment
            .get(2)
            .expect("`geometry` field must be present");
        ...
        let bridge_centroid = geometry
            .centroid()
            .expect("a centroid should exist for any non-empty geometry");
        ...
        let bridge_area = {
            ...
            let geometry_in_meters = geometry
                .transformed_crs_to_crs("WGS84", "EPSG:3364")
    ...
    //     acceptable_walkabout_bridges[3],
    //     Point::new(-75.22563703858332, 40.071892693259315)
    // );
}
